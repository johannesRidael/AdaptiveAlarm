        ...
        public override void CollectDataPoint()
        {
            if (Shell.Current.IsFocused) // App is in the foreground, get a datapoint
            {
                ...
                if (!string.IsNullOrWhiteSpace(result))
                {
                    ...
                    {
                        result = Shell.Current.DisplayPromptAsync("Wakefulness", "Please input a number 1-10", placeholder: "Scale 1-10 where 10 is best", maxLength: 2, keyboard: Keyboard.Numeric).Result;
                        if (!string.IsNullOrWhiteSpace(result))
                        {
                            result = result.Trim();
                        }
